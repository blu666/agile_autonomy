...
class TrajectoryCostLoss(nn.Module):
    ...
    def forward(self, ids_and_states, y_pred):
        ...
        for k in range(batch_size):
            traj_costs.append(self._compute_traj_cost(ids[k], states[k], y_pred[k, :, 1:]).detach())
        traj_costs = torch.stack(traj_costs).squeeze(2)
    ...
    def _compute_traj_cost(self, exp_id, state, pred_trajectories):
        ...
        for k in range(num_modes):
            traj_cost = self._compute_single_traj_cost(exp_id, state, pred_trajectories[k])
            costs.append(traj_cost)
        costs = torch.stack(costs)
        return costs

    def _compute_single_traj_cost(self, exp_id, state, trajectory):
        pcd_idx = int(exp_id.item())  # matlab indexes
        pcd_tree = self.pointclouds[pcd_idx - 1]
        traj_np = trajectory.cpu().detach().numpy()
...
class MixtureSpaceLoss(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        for j in range(self.modes):
            ...
            alpha = y_pred[:, j, 0].view(-1, 1)
            pred = y_pred[:, j, 1:].view(-1, pred_len - 1)
            mode_loss = []
            for k in range(y_true.shape[1]):  # number of traj
                mode_loss.append(self.space_loss(y_true[:, k], pred))
            mode_loss = torch.cat(mode_loss, dim=1)  # [B,K]
            mode_loss = mode_loss.unsqueeze(-1)
            alphas.append(alpha)
            mode_losses.append(mode_loss)

        alphas = torch.cat(alphas, dim=-1)  # [B,M]
        mode_losses = torch.cat(mode_losses, dim=-1)  # [B,K,M]
        max_idx = torch.argmin(mode_losses, dim=-1)  # [B,K]
        epsilon = 0.05
        loss_matrix = torch.zeros_like(alphas)
        for k in range(y_true.shape[1]):
            selection_matrix = torch.nn.functional.one_hot(max_idx[:, k], num_classes=self.modes)
            loss_matrix = loss_matrix + (selection_matrix.float() * mode_losses[:, k, :] * (1-epsilon))

        # considering all selected modes over all possible gt trajectories
        final_selection_matrix = (loss_matrix > 0.0).float()  # [B,M]
        ...
        trajectory_loss = torch.mean(torch.mean(final_cost_matrix, dim=-1))
...
class DiscretePositionLoss(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        """
        # Compute normalization factor (square length of the gt traj)
        # print(y_true.device)
        # print("---")
        # print(y_pred.device)
        average_loss = self.mse_loss(y_true, y_pred)
        average_loss = average_loss.unsqueeze(-1)
        # Compute the batch average loss. Multiplication by 10 seem to help optimization
