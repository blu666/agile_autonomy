...
class PlanDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        gt_traj = self.load_trajectory(idx)
        traj_num = self.traj_idx_num[idx]
        imu_seq = []
        img_seq = []
        depth_seq = []
        idx_seq = self.processed_idxs[idx]
        rollout_id = self.imu_obs[idx_seq[0], -1]
        for i in reversed(range(self.config.seq_len)):
            imu_o = self.imu_obs[idx_seq[i], :-1]
            imu_seq.append(imu_o)
            if self.config.use_rgb:
                img = self.decode_img_cv2(idx_seq[i])
                img_seq.append(img)
            if self.config.use_depth:
                depth = self.decode_depth_cv2(idx_seq[i])
                depth_seq.append(depth)
        imu_seq = torch.stack([torch.from_numpy(imu) for imu in imu_seq])
        if self.config.use_rgb and self.config.use_depth:
            img_seq = torch.stack([torch.from_numpy(img) for img in img_seq])
            depth_seq = torch.stack([torch.from_numpy(depth) for depth in depth_seq])
            frame_seq = (img_seq, depth_seq)
        elif self.config.use_rgb and not self.config.use_depth:
            frame_seq = torch.stack([torch.from_numpy(img) for img in img_seq])
        elif self.config.use_depth and not self.config.use_rgb:
            frame_seq = torch.stack([torch.from_numpy(depth) for depth in depth_seq])
        else:
    ...
    def preprocess_dataset(self):
        ...
        for k in range(self.imu_obs.shape[0]):
            if k % 3000 == 0:
                print(f"Built {(k / self.imu_obs.shape[0]) * 100:.2f}% of the dataset", end='\r')
            ...
            while len(idx_seq) < self.config.seq_len:
                if k - idx < 0:
                    ...
                    continue
                current_idx = k - idx
                rollout_idx = self.imu_obs[current_idx, -1]
                if rollout_idx == current_rollout_idx:
                    idx_seq.append(current_idx)
                else:
                    idx_seq.append(idx_seq[-1])
                idx += int(input_frequency / self.config.seq_len)
    ...
    def build_dataset(self):
        self.imu_obs = np.stack(self.imu_obs).astype(np.float32)
